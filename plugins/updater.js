...
Trex.addrex({pattern: 'up$', fromMe: true, desc: Lang.UPDATER_DESC}, (async (message, match) => {
    ...
    if (commits.total === 0) {
        ...
        commits['all'].map(
            (commit) => {
                degisiklikler += '⎝🛡️▷ [' + commit.date.substring(0, 10) + ']: ' + commit.message + ' ◁◁powered By ALpha Team▷▷\n';
            }
        );
...
Trex.addrex({pattern: 'updark$', fromMe: true, desc: Lang.UPDATE_NOW_DESC}, (async (message, match) => {
    await git.fetch();
    var commits = await git.log([Config.BRANCH + '..origin/' + Config.BRANCH]);
    if (commits.total === 0) {
